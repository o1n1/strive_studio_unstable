...
export async function registerUser(formData) {
  try {
    ...
    logger.log('📊 Respuesta de Supabase Auth recibida')
    ...
    logger.success('Usuario creado:', userId)

    logger.log('📝 Completando registro...')
    
    const result = await postJSON('/api/complete-registration', {
      userId: userId,
      email: formData.email,
    ...
    logger.log('📊 Resultado de completar registro:', result.success ? '✅' : '❌')
...
export async function loginUser(email, password) {
  try {
    ...
    logger.log('📊 Respuesta de login recibida')
    ...
    logger.success('Sesión iniciada:', authData.user.id)
    ...
    logger.log('📊 Perfil obtenido:', profileError ? '❌' : '✅')
    ...
    logger.success('Login exitoso - Rol:', profile.rol)
